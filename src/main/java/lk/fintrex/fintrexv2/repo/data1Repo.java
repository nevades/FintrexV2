...
public interface data1Repo extends CrudRepository<data1, Integer> {
    ...
    Iterable<SlimSelectDTO> getAllColOfficers(@Param("search") String search);
    ...
    @Query("SELECT SUM(r3l) AS r3l, SUM(r2l) AS r2l, SUM(r1l) AS r1l, SUM(r0l) AS r0l, SUM(r3le) AS r3le, SUM(r2le) AS r2le, SUM(r1le) AS r1le, SUM(r0le) AS r0le, SUM(r3l + r2l + r1l + r0l) AS rt, SUM(r3le + r2le + r1le + r0le) AS re FROM (SELECT CASE WHEN d1.AGE >= 3 AND d2.AGE < 3 THEN 1 ELSE 0 END AS r3l, CASE WHEN d1.AGE >= 2 AND d1.AGE < 3 AND d2.AGE < 2 THEN 1 ELSE 0 END AS r2l, CASE WHEN d1.AGE >= 1 AND d1.AGE < 2 AND d2.AGE < 1 THEN 1 ELSE 0 END AS r1l, CASE WHEN d1.AGE > 0 AND d1.AGE < 1 AND d2.AGE = 0 THEN 1 ELSE 0 END AS r0l, ROUND(CASE WHEN d1.AGE >= 3 AND d2.AGE < 3 THEN d1.exposure ELSE 0 END, 0) AS r3le, ROUND(CASE WHEN d1.AGE >= 2 AND d1.AGE < 3 AND d2.AGE < 2 THEN d1.exposure ELSE 0 END, 0) AS r2le, ROUND(CASE WHEN d1.AGE >= 1 AND d1.AGE < 2 AND d2.AGE < 1 THEN d1.exposure ELSE 0 END, 0) AS r1le, ROUND(CASE WHEN d1.AGE > 0 AND d1.AGE < 1 AND d2.AGE = 0 THEN d1.exposure ELSE 0 END, 0) AS r0le FROM data1 d1 JOIN data2 d2 ON d1.`FINANCE NO` = d2.`FINANCE NO` WHERE d2.`COLL OFFICER` = :user) AS subquery")
    Iterable<LoadRedCountDTO> getRedCount(@Param("user") String user);

    @Query("SELECT ROUND(SUM(CASE WHEN data1.AGE >= 3 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN 1 ELSE 0 END), 0) AS countage3to0, ROUND(SUM(CASE WHEN data1.AGE >= 2 AND data1.AGE < 3 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN 1 ELSE 0 END), 0) AS countage2to0, ROUND(SUM(CASE WHEN data1.AGE >= 1 AND data1.AGE < 2 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN 1 ELSE 0 END), 0) AS countage1to0, ROUND(SUM(CASE WHEN data1.AGE > 0 AND data1.AGE < 1 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN 1 ELSE 0 END), 0) AS countage0to0, ROUND(SUM(CASE WHEN data1.AGE >= 3 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN data1.exposure ELSE 0 END), 0) AS exposure_age3to0, ROUND(SUM(CASE WHEN data1.AGE >= 2 AND data1.AGE < 3 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN data1.exposure ELSE 0 END), 0) AS exposure_age2to0, ROUND(SUM(CASE WHEN data1.AGE >= 1 AND data1.AGE < 2 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN data1.exposure ELSE 0 END), 0) AS exposure_age1to0, ROUND(SUM(CASE WHEN data1.AGE > 0 AND data1.AGE < 1 AND data2.AGE = 0 AND data2.`COLL OFFICER` = :user THEN data1.exposure ELSE 0 END), 0) AS exposure_age0to0 FROM data1 JOIN data2 ON data1.`FINANCE NO` = data2.`FINANCE NO` WHERE data2.`COLL OFFICER` = :user")
